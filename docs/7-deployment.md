...
- **server**: The Node.js backend. The application is distributed via an immutable Docker image, automatically
  pulled from the project's _Container Registry_ (`ghcr.io`). This ensures that the running code is identical to the
  code validated by the Continuous Integration pipeline. Configuration occurs exclusively at _runtime_ via environment
  variables.
