  ...
  try {
    ...
    if (data.plan.slots.length > 0) {
      botResponse.callToAction = {
        ...
        action: () => {
          console.log("Navigazione al piano:", data.plan);  //TODO: link to the plan view
        },
        icon: {
  ...
  <div class="flex flex-col h-screen bg-base-background max-w-5xl mx-auto">
    ...
    >
      ...
      <div v-if="isLoading" class="flex items-start gap-3 mb-5 w-full">
        <img src="/icons/bot-avatar.png" alt="Bot" class="w-12 h-12 rounded-full object-cover opacity-60">
        <div class="bg-base-background rounded-2xl rounded-tl-sm p-4 flex items-center gap-1 h-12 border border-ui-border animate-pulse">
          <div class="w-2 h-2 bg-base-text rounded-full animate-bounce"></div>
          <div class="w-2 h-2 bg-base-text rounded-full animate-bounce delay-100"></div>
          <div class="w-2 h-2 bg-base-text rounded-full animate-bounce delay-200"></div>
        </div>
      </div>
    </main>
