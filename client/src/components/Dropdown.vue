...
<template>
  ...
  >
    ...
    >
      <span class="truncate">
        {{
          modelValue
            ? getLabel(
                options.find((o) => getValue(o) === modelValue) ||
                  modelValue.toString(),
              )
            : placeholder
        }}
    ...
    <div
      ...
      :id="listboxId"
      role="listbox"
      class="absolute z-50 mt-1 w-full bg-ui-card rounded-2xl shadow-xl border border-ui-border max-h-60 overflow-y-auto overflow-hidden"
    >
      <ul class="py-1">
        <li
          ...
          @click="select(option)"
          class="px-5 py-3 cursor-pointer text-base-text transition-colors duration-150"
          :class="{
