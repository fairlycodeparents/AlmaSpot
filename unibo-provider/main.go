...
type Course struct {
	AnnoAccademico  string `json:"anno_accademico"`
...
func main() {
    ...
    r.GET("/api/v1/activities", getActivities)
    ...
    go func() {
        ...
        if _, err := getCourses(); err != nil {
            fmt.Println("Failed to pre-fetch: %v\n", err)
        } else {
...
func getActivitiesHandler(c *gin.Context) {
    ...
    for _, course := range courses {
        ...
        curricula, err := timetable.GetCurricula(course.Url)
        if err != nil { continue }

        for year, currList := range curricula {
            for _, curr := range currList {
                tt, err := timetable.GetTimetable(course.Url, year, curr, interval)
                if err != nil { continue }
                ...
                for _, event := range tt {
                    ...
                    for _, room := range event.Classrooms {
                        results = append(results, ActivityResponse{
                            ...
                            Professors: []string{event.Teaches},
...
func getCourses() ([]Course, error) {
	...
	if !found { return nil, fmt.Errorf("resource not found") }

	resp, err := httpClient.Get(resource.URL)
	if err != nil { return nil, err }
	...
	for _, c := range allCourses {
		if strings.Contains(c.AnnoAccademico, yearStr) {
